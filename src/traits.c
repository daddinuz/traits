...
#define STREAM \
    ((NULL != __gStream) ? __gStream : stdout)

static logger_t *__gLauncherLogger = NULL;
#define LAUNCHER_LOGGER (__gLauncherLogger == NULL) ? __gLauncherLogger = stream_logger_new("Test", LOG_LEVEL_DEBUG, STREAM) : __gLauncherLogger

static logger_t *__gReportLogger = NULL;
#define REPORT_LOGGER (__gReportLogger == NULL) ? __gReportLogger = stream_logger_new("Report", LOG_LEVEL_DEBUG, STREAM) : __gReportLogger

static logger_t *__gAssertionLogger = NULL;
#define ASSERTION_LOGGER (__gAssertionLogger == NULL) ? __gAssertionLogger = stream_logger_new("Assertion", LOG_LEVEL_DEBUG, STREAM) : __gAssertionLogger

#define INSPECT(_Format, _Type) \
    "%" _Format " (" str(_Type) ")"

#define DUMP(_Logger, _File, _Line, _Message, ...)                                  \
    do {                                                                            \
 ...
 *
 */
static bool __gCleanupRegistered = false;

static void __cleanup(void) {
    logger_delete(&__gLauncherLogger);
    logger_delete(&__gReportLogger);
    logger_delete(&__gAssertionLogger);
}

#define REGISTER_CLEANUP_FUNCTION       \
    if (!__gCleanupRegistered) {        \
        atexit(__cleanup);              \
        __gCleanupRegistered = true;    \
    }
...
static void __launch(const char *name, test_t test) {
    ...
    if (NULL != test) {
        log_notice(LAUNCHER_LOGGER, "Running '%s'\n", name);
        test();
        if (__gState <= 0) {
            log_info(LAUNCHER_LOGGER, "Success '%s'\n", name);
        } else {
            log_fatal(LAUNCHER_LOGGER, "Failed '%s'\n", name);
            __gFailed += 1;
        }
    } else {
        log_warning(LAUNCHER_LOGGER, "Skipping '%s'\n", name);
        __gSkipped += 1;
    }
}
...
int report(void) {
    REGISTER_CLEANUP_FUNCTION
    if (__gFailed > 0) {
        log_fatal(REPORT_LOGGER, "%s: %d, %s: %d, %s: %d, %s: %d\n", "Tests", __gRun, "Succeeded",
                  __gRun - (__gFailed + __gSkipped), "Failed", __gFailed, "Skipped", __gSkipped);
    } else if (__gSkipped > 0) {
        log_warning(REPORT_LOGGER, "%s: %d, %s: %d, %s: %d, %s: %d\n", "Tests", __gRun, "Succeeded",
                    __gRun - (__gFailed + __gSkipped), "Failed", __gFailed, "Skipped", __gSkipped);
    } else {
        log_info(REPORT_LOGGER, "%s: %d, %s: %d, %s: %d, %s: %d\n", "Tests", __gRun, "Succeeded",
                 __gRun - (__gFailed + __gSkipped), "Failed", __gFailed, "Skipped", __gSkipped);
    }
    return __gFailed;
}
...
static void __traits_bool(__operator_t op, const bool expected, const bool got, const char *file, int line) {
    switch (op) {
        __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_EQUAL, EXPR_EQUAL(expected, got), "s", bool,
                                bool2str(expected), bool2str(got));
...
static void __traits_ptr(__operator_t op, const void *expected, const void *got, const char *file, int line) {
    switch (op) {
        __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_EQUAL, EXPR_EQUAL(expected, got), "p", void *, expected,
                                got);
        __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_NOT_EQUAL, EXPR_NOT_EQUAL(expected, got), "p", void *,
                                expected, got);
...
static void __traits_str(__operator_t op, const char *expected, const char *got, const char *file, int line) {
    switch (op) {
        __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_EQUAL, EXPR_EQUAL(0, strcmp(expected, got)), "s", char *,
                                expected, got);
        __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_NOT_EQUAL, EXPR_NOT_EQUAL(0, strcmp(expected, got)), "s",
                                char *, expected, got);
...
#define INTEGER_DEFINE(_Type, _Identifier, _Format)                                                                                              \
    static void __traits_##_Type(__operator_t op, const _Type delta, const _Type expected, const _Type got, const char *file, int line) {        \
        switch (op) {                                                                                                                            \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_EQUAL, EXPR_EQUAL(expected, got), _Format, _Type, expected, got);                 \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_NOT_EQUAL, EXPR_NOT_EQUAL(expected, got), _Format, _Type, expected, got);         \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_GREATER_EQUAL, EXPR_GREATER_EQUAL(expected, got), _Format, _Type, expected, got); \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_GREATER, EXPR_GREATER(expected, got), _Format, _Type, expected, got);             \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_LESS_EQUAL, EXPR_LESS_EQUAL(expected, got), _Format, _Type, expected, got);       \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_LESS, EXPR_LESS(expected, got), _Format, _Type, expected, got);                   \
            __HANDLE_DELTA_OPERATOR(ASSERTION_LOGGER, OPERATOR_WITHIN, EXPR_WITHIN(delta, expected, got), _Format, _Type, expected, got, delta); \
...
#define FLOATING_DEFINE(_Type, _Identifier, _Format)                                                                                                          \
    static void __traits_##_Type(__operator_t op, const _Type delta, const _Type expected, const _Type got, const char *file, int line) {                     \
        switch (op) {                                                                                                                                         \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_EQUAL, EXPR_WITHIN(PRECISION(str(_Type)), expected, got), _Format, _Type, expected, got);      \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_NOT_EQUAL, !EXPR_WITHIN(PRECISION(str(_Type)), expected, got), _Format, _Type, expected, got); \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_GREATER_EQUAL, EXPR_GREATER_EQUAL(expected, got), _Format, _Type, expected, got);              \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_GREATER, EXPR_GREATER(expected, got), _Format, _Type, expected, got);                          \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_LESS_EQUAL, EXPR_LESS_EQUAL(expected, got), _Format, _Type, expected, got);                    \
            __HANDLE_BASIC_OPERATOR(ASSERTION_LOGGER, OPERATOR_LESS, EXPR_LESS(expected, got), _Format, _Type, expected, got);                                \
